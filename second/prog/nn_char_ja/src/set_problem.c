...
void set_problem(){
  ...
  for(count=0; count<CTG; count++){
	...
	c[1] = '\0';
        if( atoi(c) == 1 ){
            i_lay[count][(count_y*SIZE_Y)+count_x] = ON;
            fprintf(stdout,"1");
        }else if( atoi(c) == 0 ){
            i_lay[count][(count_y*SIZE_Y)+count_x] = OFF;
            fprintf(stdout,"0");
        }else{
      ...
      i_lay[count][(count_y*SIZE_Y)+count_x] = ON; /* bias */
      fprintf(stdout,"\n");
    }/* end of for(count2) */
...
fprintf(stderr,"# ERROR! fopen(%s) was failed  in nn_check().\n",filename);
  ...
  for(count_y=0; count_y<SIZE_Y; count_y++){
   fflush(stdout);
    ...
    for(count_x=0; count_x<SIZE_X; count_x++){
      ...
      c[1] = '\0';
      if( atoi(c) == 1 ){
          test_pattern[(count_y*SIZE_Y)+count_x] = ON;
          fprintf(stdout,"1");
      }else if( atoi(c) == 0 ){
          test_pattern[(count_y*SIZE_Y)+count_x] = OFF;
          fprintf(stdout,"0");
      }else{
  ...
  test_pattern[(count_y*SIZE_Y)+count_x] = ON; /* bias */
